...
function VirtualSky(input){
	...
	this.background = "rgba(44 64 75,1)";	// Default background colour is transparent
	this.color = "rgba(255,255,255,1)";					// Default background colour is chosen automatically
...
VirtualSky.prototype.drawImmediate = function(proj){
	...
	if(this.polartype){
		...
		c.closePath();
		if(!this.transparent){
			c.fillStyle = (this.background) ? this.background : ((this.negative) ? white : black);
			c.fill();
		}
		c.lineWidth = 0.5;
		c.strokeStyle = black;
	...
	c.fillStyle = 'rgba(0,0,0,0)';//txtcolour;
	c.lineWidth = 1.5;
	this.setFont();
...
VirtualSky.prototype.drawConstellationLines = function(colour){
	...
	if(!colour) colour = this.color;
	var x = this.ctx;
	x.beginPath();
	x.strokeStyle = colour;
	x.fillStyle = colour;
	x.lineWidth = 1;//(this.constellation.lineWidth || 0.75);
	var fontsize = this.fontsize();
	this.setFont();
...
VirtualSky.prototype.drawGridlines = function(type,step,colour){
	...
	c.beginPath();
	c.strokeStyle = this.color;
	c.lineWidth = 1;//(this.grid.lineWidth || 1);
	bstep = 2;
	if(type=="az"){
